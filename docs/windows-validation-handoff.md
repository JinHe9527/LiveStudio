...
最大化窗口会让扩展标题栏的存档操作区失去可点击空间，因此桌面窗口现明确取消最大化能力：Windows 原生样式移除最大化按钮，双击标题栏、窗口吸附、系统快捷键或外部窗口请求触发最大化时均立即恢复为普通窗口；使用者仍可拖动窗口边缘调整普通窗口大小。真机对窗口句柄检查确认 `WS_MAXIMIZEBOX` 已清除，强制发送最大化请求后 `IsZoomed=False`，请求前后窗口尺寸均为 1256×788。
