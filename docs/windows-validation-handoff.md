...
> `v0.1.31` 已按使用者要求正式发布（提交 `b3314a4`，Release `34854758133` 成功）。407 项测试、签名与安装器自检通过，公开更新服务发现新版本及退役地址跳过检查通过。main/标签 CI 的 Windows 检查通过，MinIO 拉取仍失败。本机未安装此次更新；详见 [更新路径审查发布记录](update-path-audit-2026-09-14.md#v0131-发布记录)。

> 2026-09-14 更新连接审查见 [update-path-audit-2026-09-14.md](update-path-audit-2026-09-14.md)。实际安装版 0.1.29 清单元数据为空，使用 GitHub 更新；旧腾讯云清单在本机直连和代理下均 TLS 失败。使用者确认腾讯云已撤销，源码直接跳过该旧地址，并补齐最终资产重定向检查、超时和取消反馈，407 项测试通过，尚未发布；另一台电脑的拒绝连接根因未取得现场证据，不能宣称已修复。

> 2026-09-08 当前修复记录见 [delivery-audit-2026-09-08.md](delivery-audit-2026-09-08.md)。本机直播伴侣已更新至 12.9.3.475022132；新增 2522 字段的精确签名矩阵与写入前结构检查，修复空值遗漏、数组尾项残留、多实例误报成功和权威摄像头文件权限预检。真实美白单变量恢复与 20 次同状态联合恢复通过。正式发布和故障结果以新报告的完成记录为准；多路不同配置、data2、矩阵外字段和跨实体采集卡验收仍未完成，Verified=0。

> 2026-09-05 最终交付记录见 [delivery-audit-2026-09-05.md](delivery-audit-2026-09-05.md)。v0.1.25 已正式发布，CI 与签名发布全通过；本机由 0.1.17 升级至本地签名构建 0.1.25，已安装 Agent 恢复与回读通过，软件内更新显示最新版。345 项 .NET 测试、6 项依赖门测试、本机摄像头删除重建、7 阶段回滚、2 项崩溃恢复和 5 次连续恢复通过。当前 45 份包有效、2449 字段稳定。多摄像头不同配置仍不支持，全量插件/字段预检与跨实体采集卡证据仍有缺口，Verified=0，不得描述为全目标场景完整交付。
...
提交 `982b2d3` 的普通 CI `33889155747` 已通过 Windows Release 构建、322 项测试、格式检查、依赖漏洞检查，以及隔离 PostgreSQL/MinIO 集成测试。`v0.1.23` Release 工作流 `33889557743` 随后完成相同质量门、固定证书导入、签名 MSIX、签名一键安装器、自检和 GitHub Release 创建，全部成功；发布流程中已不存在腾讯云镜像等待或回下载步骤。公开 `LiveStudio-Setup.exe` 长度为 217874832 字节，GitHub Asset Digest 与侧车 SHA-256 均为 `8B0FECE59616062CB05C85B20BB2493D7737805EC6B90B2395EA43C3447FCB65`；MSIX 长度为 159149317 字节，SHA-256 为 `58B605774ED2A19E885C26A200AA740169F4EC0D864CECBB54B56369C66AEC42`。

使用 Release 编译的更新服务进行真实匿名 HTTPS 检查：当前版本设为 0.1.22 时返回 `v0.1.23` 及 GitHub 官方安装器、校验地址；当前版本设为 0.1.23 时返回没有更新。由此证明新客户端不依赖腾讯云更新清单即可发现后续 GitHub Release；即时下载速度仍取决于使用者网络到 GitHub CDN 的路由，不能仅凭发布成功宣称固定带宽。

## 48. 2026-09-08 v0.1.26 字段完整性修复与安装复验

本轮完整记录见 `docs/delivery-audit-2026-09-08.md`。直播伴侣已自更新至 `12.9.3.475022132`；依据当前真实结构新增 2522 项签名字段矩阵，并强制目标路径/类型完整匹配。修复空值丢失、数组旧尾项、重复设备配置覆盖、只检查部分摄像头回读、摄像头载荷缺少预检等问题。`data2`、不同配置的多摄像头和矩阵外字段仍明确阻断，不能宣称已支持。

359 项自动化测试、依赖门、格式与整仓构建通过。当前单机完成一次真实美白单变量恢复、20 次同状态恢复稳定性测试、7 阶段故障回滚和 2 个真实 Agent 崩溃恢复检查。20 次同状态测试不等于 20 次保存/修改/恢复完整循环，当前只有 OBS Virtual Camera，尚无第二台不同实体采集卡证据，因此 `Verified=0`。

代码提交 `35e6b32`，`v0.1.26` Release `34245075109` 与两个 CI 均成功。本机本地签名包升级至 `0.1.26.0`，安装前后 78 份存档逐文件不变；安装版再完成一次真实联合恢复，25.721 秒，残留为 0。最终 79 份包签名/哈希有效，2522 项回读无差异。本轮新增 34 份测试存档保留在本机并关闭其同步资格，原有 45 份与用户凭据不变。

固定 GitHub 下载链接返回 HTTP 200 并指向 `v0.1.26`，软件内更新显示当前为最新版本。公开 Setup 为 217934232 字节，SHA-256 `2a1e7ea14732ff899e7ef75dc6d8b354278a21b44e70e51c5d8164f6e411dbba`；公开 MSIX 为 159208845 字节，SHA-256 `555080459738e665ae2178d09922d575926953450505ba9c8b059c8fa0279787`。本机未完整回下载公开 EXE，不以本地签名包替代公开字节验证；Release Runner 的正式签名与载荷自检均通过。

## 49. 2026-09-10 OBS 连接失败与按钮锁定修复（未发布）

使用者报告另一台电脑未开启 OBS WebSocket，保存时报就绪超时，随后按钮无法操作；要求提供自动连接，也允许每台电脑自行填写连接信息。本轮基于 `main / 8d311a7`，开始时工作区干净。对方电脑的具体 OBS/LiveStudio 版本、安装方式及现场日志尚未提供，不能断言已复现该电脑全部故障。

代码确认并修复以下问题：

- 后台 `IsBusy` 是瞬时状态，原先直接赋给桌面 `IsBusy`，没有后续刷新就可能永久锁住命令；现在桌面忙碌状态仅由本次桌面操作管理，执行端资格和操作锁继续阻止并发写入。后台空闲状态也不能提前解锁进行中的桌面任务。
- OBS 握手、请求和关闭握手原先存在无限等待；现在分别设置响应和关闭时限。自动连接使用与适配器相同的总期限及单次期限重试，手动/自动连接和桌面状态请求也具备有界等待。恢复事务仍不使用普通连接请求的桌面超时。
- OBS 进程存在但 WebSocket 不可读时，原先显示“已连接”；现在显示“WebSocket 未连接”。存档页在未连接时直接提供自动连接与手动连接设置入口，自动连接结果同步显示于存档页。
- 设置页提供自动连接、服务器地址（含端口）、遮蔽密码输入、验证并保存以及重新检测。验证成功后才保存本机凭据；验证失败保留输入和已有连接配置。状态协议只回传本机已保存地址，不回传密码，普通刷新不覆盖用户正在修改的地址。
- 自动连接继续读取当前 Windows 用户的 OBS 配置，不使用开发机密码；首次缺少配置文件时先尝试启动 OBS 等待生成。无法操作 OBS 设置窗口时明确提示处理弹窗、权限或改用手动连接，不宣称任何安装形式都可自动开启。

验证：Release 整仓构建 0 警告、0 错误；Setup 8、Agent 51、Core 311，共 370 项测试通过；直接与传递依赖无已知漏洞；格式验证及 `git diff --check` 通过。新增 11 项回归覆盖忙碌状态、地址编辑保留、非法地址、运行中 WebSocket 不可读，以及本机模拟 WebSocket 服务不发 Hello、不响应请求、不响应关闭和调用方取消。
...
## 53. 2026-09-15 异机 data2 保存与配置读取修复

使用者报告两台最新版电脑分别因 data2 摄像头容器及未找到配置而保存失败，并明确要求修复、检查、提交更新。修复及证据边界见 [异机审查记录](cross-machine-audit-2026-09-15.md)。枚举、签名绑定、载荷写入与逐实例回读现保留 data/data2 容器；目标画布上下文不参与可移植画面恢复。增加有界配置目录发现、明确读取失败分类、拒绝半份读取，以及保存/恢复一致的版本结构资格。

本机只读原生树和内存副画布投影可匹配签名结构；隔离文件回归不替代两台原故障电脑的实际恢复。不同设备/不同配置多摄像头、副画布原生 UI 重建及未识别后端仍有边界。未改当前生产配置，Verified=0。最终测试和发布结果以审查记录末节为准。


53 节发布结果：0826823 / v0.1.32 已推送，Release 34965186158 成功，427 项测试和固定身份签名/安装器自检通过；真实更新服务验证 0.1.31 能发现 0.1.32。两个 CI 的 Windows 检查通过，MinIO 启动失败使云集成未执行。本机公开安装器回下载在 3 分钟期限取消，部分文件已清理，未安装。详见异机审查记录的发布小节；不能宣称两台故障电脑已完成恢复验收。

## 54. 2026-09-15 自动错误报告实现（接收服务待部署）

使用者要求把异机无法使用的问题自动提交到其 GitHub。新增共享 Diagnostics、持久去重队列、Desktop/Agent/Setup 异常入口、恢复事务失败记录和设置页开关；新增独立 .NET 接收端，固定提交到 JinHe9527/LiveStudio Issues。报告采用异常类型、HResult、固定原因码和代码调用位置，不上传原始异常文本、用户路径、账号或直播配置。详见 `automatic-error-reporting.md`。

接收端支持未知字段拒绝、正文大小和频率限制、持久去重、同类报告合并、维护者笔记保留，以及不确定 GitHub POST 的核对。Release workflow 已支持通过仓库变量 `LIVESTUDIO_DIAGNOSTICS_ENDPOINT` 同时向三个进程注入 HTTPS 地址；地址未配置时只落盘，并明确显示尚未配置接收服务。

没有部署公网接收服务、没有设置服务端专用 GitHub 凭据、没有完成真实自动提交或异机回归，也没有发布带此功能的新安装包。真实 GitHub 联调与本机服务启动测试均被自动审批策略阻止，未执行成功。不得宣称测试电脑已经自动上报。当前正式版仍为 v0.1.32，恢复能力与 `Verified=0` 的边界保持不变。

本轮本地结果：Release 构建 0 警告、0 错误；全部 446 项测试通过（Core 381、Agent 57、Setup 8，其中新增诊断测试 19 项）；格式检查、diff 检查、17 个项目的依赖漏洞门和 6 项依赖门自测通过。这些结果不替代公网与两台测试电脑上的实际提交验收。

## 55. 2026-09-15 v0.1.33 离线诊断包

使用者选择“自动记录＋一键导出诊断包”，并明确要求提交更新。新增设置页导出 ZIP，固定包含说明、脱敏环境/兼容摘要及错误记录，无需联网、额外账号或成功连接 Agent。空日志仍可导出；损坏、不可读、过大和超过数量限制的日志明确计入跳过数量；导出保留原队列。兼容检查异常补充本地记录。未配置接收地址时隐藏 GitHub 自动提交入口。

本地 Release 构建 0 警告、0 错误，455 项测试通过（Core 390、Agent 57、Setup 8），新增 9 项诊断包测试覆盖空记录、多个组件、计数保留、额外 JSON 属性排除、原始文件排除、损坏文件和兼容摘要投影。格式和 diff 检查通过；联网依赖审计及正式发布结果以本节后续记录为准。没有在原失败两台电脑上实际复测，恢复验收仍为 Verified=0。

发布完成：功能与版本提交 `61466f4bd8b1447f3e0391a3ef005d00952ff842`，标签 `v0.1.33` 已推送；Windows Release 工作流 `34978132264` 成功，正式 Release 7 个资产齐全，发布说明已更新。main/tag CI `34978132271`、`34978132613` 的 Windows 构建测试均通过；云端集成仍失败在“启动 MinIO”，不能宣称全部 CI 通过。本地 17 项目依赖漏洞检查和依赖门最终均通过。

- Setup：189728240 字节；SHA-256 `85ffb2ab30aad03b65fdc7c9a5f5440c4bfff973e4f59381a78042a459b33692`。
- MSIX：130976041 字节；SHA-256 `d87f972cd280b342ad788c9266e42d51bd2770fb482e80d3e9117040da45a7bc`。
- 已回读公开校验侧车并与 GitHub Asset Digest 核对一致；签名报告确认固定指纹 `4D42933F643E1E0B649513BCD10A15B485746E1D`。一次性 runner 的 SignTool 仅报预期的自签名根不受信任，项目内部签名验证规则通过。本轮未完整回下载或在本机安装公开 EXE。
- 使用真实 ApplicationUpdateService 匿名 HTTPS 检查：首次 0.1.32 请求因网络 30 秒超时，重试后发现 v0.1.33 和版本化安装器地址；模拟 0.1.33 返回无更新。证据位于忽略目录 `artifacts/diagnostic-package-preview/update-check-retry.json`。
- 设置页诊断卡片完成离屏渲染检查，导出按钮与说明可见；未模拟原生保存对话框操作，也未将离屏渲染当成两台电脑真机验收。
