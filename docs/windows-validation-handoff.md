...
最大化窗口会让扩展标题栏的存档操作区失去可点击空间，因此桌面窗口现明确取消最大化能力：窗口使用仅保留边框的原生装饰和自绘标题栏，右侧只显示“最小化”和“关闭”，不再渲染灰色的最大化图标；双击标题栏、窗口吸附、系统快捷键或外部窗口请求触发最大化时均立即恢复为普通窗口，使用者仍可拖动窗口边缘调整普通窗口大小。真机 UI Automation 确认“最小化”和“关闭”存在、“最大化”不存在，最小化按钮可正常调用并恢复；窗口句柄中的 `WS_MAXIMIZEBOX` 已清除，强制发送最大化请求后 `IsZoomed=False`，请求前后窗口尺寸均为 1256×788。截图为 `C:\Users\WYZB\AppData\Local\Temp\livestudio-custom-titlebar-no-maximize.png`。
