...
$resolvedSignedFilePath = [IO.Path]::GetFullPath($SignedFilePath)
...
$verificationOutput = @(& $signTool verify /pa /v $resolvedSignedFilePath 2>&1)
...
$signature = Get-AuthenticodeSignature -LiteralPath $resolvedSignedFilePath
...
if (-not $actualThumbprint -or
    ...
    throw "签名证书指纹不匹配：$actualThumbprint"
}
...
if (-not $isExpectedInternalTrustResult) {
    throw "Authenticode 签名复验失败：$($signature.Status)"
}
