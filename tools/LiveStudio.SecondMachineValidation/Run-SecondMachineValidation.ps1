...
try {
    ...
    elseif (-not $ExecuteFiveRestoreCycles) {
        ...
        if ($report.applications.liveStudioVersion -ne '0.1.23.0' -or
            $report.applications.liveStudioAgentVersion -ne '0.1.23.0') {
            throw "LiveStudio Desktop and Agent must both be 0.1.23.0. Desktop=$($report.applications.liveStudioVersion), Agent=$($report.applications.liveStudioAgentVersion)."
        }
